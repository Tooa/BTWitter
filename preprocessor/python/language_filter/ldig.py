...
# ldig_detector : Language Detector with Infinite-Gram
...
class ldig(object):
    def __init__(self, model_dir):
        ...
        self.param = os.path.join(model_dir, 'parameters3.npy')
        self.doublearray = os.path.join(model_dir, 'doublearray3.npz')
        self.t = self.load_da()
        self.p = numpy.load(self.param)
        self.l = self.load_labels()
    ...
    def detect_language(self, tweet):
        return guess_lanuage(tweet, self.p, self.l, self.t)
        #log_likely = likelihood(param, labels, trie, args, options)
...
def predict(param, events):
    sum_w = numpy.dot(param[list(events.keys()),].T, list(events.values()))
...
def guess_lanuage(tweet, param, labels, trie):
    #K = len(labels)
    #corrects = numpy.zeros(K, dtype=int)
    #counts = numpy.zeros(K, dtype=int)
    ...
    #    f = codecs.open(filename, 'rb',  'utf-8')
    #    for i, s in enumerate(f):
    label, text, org_text = normalize_text(tweet)
...
if __name__ == '__main__':
    ...
    if options.init:
        ...
        #import cProfile
        #cProfile.runctx('detector.detect(options, args)', globals(), locals(), 'ldig_detector.profile')

...
class DoubleArray(object):
    ...
    def load(self, filename):
        ...
        #from os import path
        #filename = path.join(*path.split(filename)[:-1])

        #self.base = numpy.loadtxt(path.join(filename, 'base.npy'))
        #self.check = numpy.loadtxt(path.join(filename, 'check.npy'))
        #self.value = numpy.loadtxt(path.join(filename, 'value.npy'))
        #self.N = self.base.size
